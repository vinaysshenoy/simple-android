...
class BloodPressureRepositoryTest {
  ...
  fun `when saving a measurement, correctly get IDs for the current user and facility`() {
    ...
    whenever(encounterRepository.save(any())).thenReturn(Completable.complete())
    ...
    verify(encounterRepository).save(check { observations ->
      val measurement = observations.first().bloodPressures.first()
